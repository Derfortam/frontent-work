...
function checkMonth(month) {
   ...
   if (month > 12 || month < 1) {
      ...
      setDataError(monthInput, monthLabel, "Некоректна дата", "month-error")
   } else {
...
function checkYear(year) {
   ...
   if (year > currentYear || year < 0) {
      ...
      setDataError(yearInput, yearLabel, "Некоректна дата", "year-error")
   } else {
      console.log("Коректно")
...
calculateButton.addEventListener("click", (e) => {
   ...
   if (getItemValue(dayInput) && getItemValue(monthInput) && getItemValue(yearInput)) {
      ...
      // Перевіряємо, чи місяці зі зміни років при відніманні дають значення менше 0
      if (monthsDiff < 0 || (monthsDiff === 0 && currentDate.getDate() < specifiedDate.getDate())) {
         yearsDiff-- // Зменшуємо кількість років, якщо є меньше одного року до поточної дати
      }
      ...
      if (day === false) {
         setDataError(dayInput, dayLabel, "Пусте поле", "day-empty")
      } else {
         const dayError = document.querySelector("#day-empty")
         if (dayError) {
            removeDataError(dayInput, dayLabel, dayError)
      ...
      if (month === false) {
         setDataError(monthInput, monthLabel, "Пусте поле", "month-empty")
      } else {
         const monthError = document.querySelector("#month-empty")
         if (monthError) {
            removeDataError(monthInput, monthLabel, monthError)
      ...
      if (year === false) {
         setDataError(yearInput, yearLabel, "Некоректна дата", "year-empty")
      } else {
         const yearError = document.querySelector("#year-empty")
         if (yearError) {
            removeDataError(yearInput, yearLabel, yearError)
